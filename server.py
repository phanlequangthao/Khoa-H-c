...
def handle_client(conn, addr):
    ...
    while True:
        try:
            ...
            if room_code not in room_codes:
                conn.send("Invalid room code".encode('utf-8'))
                continue
    ...
    client_addresses[conn] = client_ip  # Lưu địa chỉ IP của client
    
    if len(room_codes[room_code]) == 2:
        # Gửi địa chỉ IP của mỗi người dùng cho người kia trong phòng
